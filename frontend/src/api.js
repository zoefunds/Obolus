...
export const api = {
  ...
  fundVault: (id, valueWei) => req(`/vaults/${id}/fund`, { method: "POST", body: JSON.stringify({ valueWei }) }),
  ...
  withdraw: (amountWei) => req("/balances/withdraw", { method: "POST", body: JSON.stringify({ amountWei }) }),

  pause: () => req("/admin/pause", { method: "POST", body: "{}" }),
  unpause: () => req("/admin/unpause", { method: "POST", body: "{}" }),
  setMinimumBonds: (minClaimantBondWei, minContesterBondWei) =>
    req("/admin/minimum-bonds", { method: "POST", body: JSON.stringify({ minClaimantBondWei, minContesterBondWei }) }),
